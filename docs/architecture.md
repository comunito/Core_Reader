...
| `HTTPTTSConfigStore`                 | `UserDefaults` + `KeychainService` | Feature #72 WI-1 loader: decodes the persisted `HTTPTTSConfig` from UserDefaults and splices the API key from Keychain; `loadValidConfig()` returns the config only when it passes `validate()`. Consumed by `TTSService.defaultSynthesizer` to decide whether the cloud path is active. Google Cloud stores endpoint/voice/language/speed in the non-secret config and keeps the API key only in Keychain. |
| `ReadingProgressStore`               | `ReadingPositionPersisting` + `VReaderLocatorPersisting` | Single progress boundary for visual Readium relocates and TTS updates. Exposes `ReadingProgressRecord`, serializes locator data, and provides deterministic last-write-wins resolution without activating CloudKit. |
| `AudioCache`                         | Application-support/cache filesystem + JSON sidecars | Stores synthesized audio outside SwiftData. `AudioCacheKey` includes book identity, locator/text identity, provider, voice, language, and speed; supports lookup, book purge, and full purge. |
| `AudioPlaybackService`               | `HTTPTTSChunkPlayer` | Format-neutral playback state (`idle/playing/paused`) and current chunk. Keeps UI/background-audio coordination separate from provider/network code; lock-screen controls remain deferred. |
